...
public:
    ...
    float y_cut;
    // ctor/dtor
    Sphere(float radius = 1.0f, float y_cut = -10 , int sectorCount = 36, int stackCount = 18, glm::vec3 amb = glm::vec3(1.0f, 0.0f, 0.0f), glm::vec3 diff = glm::vec3(1.0f, 0.0f, 0.0f), glm::vec3 spec = glm::vec3(0.5f, 0.5f, 0.5f), float shiny = 32.0f) : verticesStride(24)
    {
        this->y_cut = y_cut;
...
private:
    ...
    void buildCoordinatesAndIndices()
    {
        ...
        for (int i = 0; i <= stackCount; ++i)
        {
            ...
            // added by taj
            if (radius * sinf(stackAngle) >= y_cut)
            {
                y = radius * sinf(stackAngle);
            }
